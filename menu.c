...
void desenha_boneco_abaixado_andando(ALLEGRO_BITMAP *sprite_sheet, struct boneco personagem, int sprite){
    ...
    if(personagem.lado >= 0){
        ...
        if(personagem.atirando)
            al_draw_scaled_bitmap(sprite_sheet, sprite * 4 *PIXEL + PIXEL*2, ini2, PIXEL*3, PIXEL*2, personagem.x+PIXEL, personagem.y-108+5 * PIXEL, 105, 70, 0);
        else
        ...
        if(personagem.atirando)
            al_draw_scaled_bitmap(sprite_sheet, sprite * 4 *PIXEL + PIXEL*2, ini2, PIXEL*3, PIXEL*2, personagem.x-5*PIXEL-4, personagem.y-108+ 5*PIXEL, 105, 70, ALLEGRO_FLIP_HORIZONTAL);
        else
...
void desenha_boneco_pulando(ALLEGRO_BITMAP *sprite_sheet, struct boneco personagem, int sprite){
    ...
    if(personagem.lado >= 0){
        ...
        if(personagem.atirando)
            al_draw_scaled_bitmap(sprite_sheet, sprite * 4 *PIXEL + 2*PIXEL, ini2, PIXEL*3, PIXEL*2, personagem.x, personagem.y-108+4 * PIXEL, 105, 70, 0);
        else
        ...
        if(personagem.atirando)
            al_draw_scaled_bitmap(sprite_sheet, sprite * 4 *PIXEL + 2*PIXEL, ini2, PIXEL*3, PIXEL*2, personagem.x-4*PIXEL-4, personagem.y-108+4 * PIXEL, 105, 70, ALLEGRO_FLIP_HORIZONTAL);
        else
            al_draw_scaled_bitmap(sprite_sheet, 2*PIXEL, ini2, PIXEL*3, PIXEL*2, personagem.x-4*PIXEL-4, personagem.y-108+4 * PIXEL, 105, 70, ALLEGRO_FLIP_HORIZONTAL);
    }
...
void desenha_boss(struct inimigo *final_boss, struct conjunto_sprites_boss sprites_boss, int altura_boss, int largura_boss, int sprite){
    if(final_boss->atirando == 0)
        al_draw_scaled_bitmap(sprites_boss.parado, sprite*89, 0, 89, 73, final_boss->x, final_boss->y-altura_boss+10, largura_boss, altura_boss, ALLEGRO_FLIP_HORIZONTAL);
    else if(final_boss->atirando == 1){
        if(sprite == 0)
            al_draw_scaled_bitmap(sprites_boss.atirando, PIXEL, 0, 15*PIXEL, 72, final_boss->x, final_boss->y-altura_boss, largura_boss, altura_boss, ALLEGRO_FLIP_HORIZONTAL);
        else if(sprite == 1) 
            al_draw_scaled_bitmap(sprites_boss.atirando, 16*PIXEL, 0, 15*PIXEL, 72, final_boss->x, final_boss->y-altura_boss, largura_boss, altura_boss, ALLEGRO_FLIP_HORIZONTAL);
        else if(sprite == 2)
            al_draw_scaled_bitmap(sprites_boss.atirando, 32*PIXEL, 0, 15*PIXEL, 72, final_boss->x, final_boss->y-altura_boss, largura_boss, altura_boss, ALLEGRO_FLIP_HORIZONTAL);
        else if(sprite == 3)
            al_draw_scaled_bitmap(sprites_boss.atirando, 47*PIXEL, 0, 14.75*PIXEL, 72, final_boss->x, final_boss->y-altura_boss, largura_boss, altura_boss, ALLEGRO_FLIP_HORIZONTAL);
    }
    else if(final_boss->atirando == 2){
        if(sprite <= 1)
            al_draw_scaled_bitmap(sprites_boss.atirando_cima, sprite*15*PIXEL, 0, 15*PIXEL, al_get_bitmap_height(sprites_boss.atirando_cima), final_boss->x, final_boss->y-altura_boss, largura_boss, altura_boss, ALLEGRO_FLIP_HORIZONTAL);
        else if(sprite == 2) 
            al_draw_scaled_bitmap(sprites_boss.atirando_cima, 33*PIXEL, 0, 15.875*PIXEL, al_get_bitmap_height(sprites_boss.atirando_cima), final_boss->x, final_boss->y-altura_boss, largura_boss, altura_boss, ALLEGRO_FLIP_HORIZONTAL);
        else if(sprite == 3)
            al_draw_scaled_bitmap(sprites_boss.atirando_cima, 49*PIXEL, 0, 14.75*PIXEL, al_get_bitmap_height(sprites_boss.atirando_cima), final_boss->x, final_boss->y-altura_boss, largura_boss, altura_boss, ALLEGRO_FLIP_HORIZONTAL);
        }
}

void desenha_boss_parado(struct inimigo *final_boss, struct conjunto_sprites_boss sprites_boss, int altura_boss, int largura_boss, int sprite){
    if(final_boss->lado)
        al_draw_scaled_bitmap(sprites_boss.parado, sprite*89, 0, 89, 73, final_boss->x, final_boss->y-altura_boss+10, largura_boss, altura_boss, ALLEGRO_FLIP_HORIZONTAL);
    else
        al_draw_scaled_bitmap(sprites_boss.parado, PIXEL/2, PIXEL/2, 11*PIXEL_BOSS, 9*PIXEL_BOSS, final_boss->x, final_boss->y-altura_boss+10, largura_boss, altura_boss, 0);

}

void desenha_boss_andando(struct inimigo *final_boss, struct conjunto_sprites_boss sprites_boss, int altura_boss, int largura_boss, int sprite){
    if(final_boss->lado)
        al_draw_scaled_bitmap(sprites_boss.andando, sprite*89, 0, 89, 73, final_boss->x, final_boss->y-altura_boss+10, largura_boss, altura_boss, ALLEGRO_FLIP_HORIZONTAL);
    else
        al_draw_scaled_bitmap(sprites_boss.andando, PIXEL/2, PIXEL/2, 11*PIXEL_BOSS, 9*PIXEL_BOSS, final_boss->x, final_boss->y-altura_boss+10, largura_boss, altura_boss, 0);
}
...
void desenha_bala(struct arma *gun, int distancia_andada, int bala){
    struct bala *bullet;
    ...
    while(bullet != NULL){
        ...
        if(bala == 0)
            ...
            al_draw_filled_rectangle(x, y, x+20, y+7, al_map_rgb(0, 0, 150));
    ...
    for(int i=0;i<MAX_OBSTACULOS;i++){
        ...
        if(obs_tela2 >= 0 && obs_tela1 <= X_SCREEN){
            ...
            if(estruturas[i].inimigo){
                ...
                if(temp->vida <= 0){
                    ALLEGRO_BITMAP *explosion = al_load_bitmap("usaveis/explosion4.png");
                    verifica_init(explosion, "explosion");
                    al_draw_scaled_bitmap(explosion, 0, 0, al_get_bitmap_width(explosion), al_get_bitmap_height(explosion), temp->x-105-distancia_andada, temp->y-75, 280, 210, 0);
...
void desenha_boss_fight(struct arma *gun, struct boneco *personagem, struct inimigo *final_boss, struct obstacle estrutura_boss, struct conjunto_sprites_boss sprites_boss,
    ALLEGRO_BITMAP *fundo_menu, ALLEGRO_BITMAP *sprite_sheet,  int sprite, int altura_boss, int largura_boss, int X_SCREEN, int Y_SCREEN){
    al_draw_scaled_bitmap(fundo_menu, 0, 0, al_get_bitmap_width(fundo_menu), al_get_bitmap_height(fundo_menu), 0, 0, X_SCREEN, Y_SCREEN, 0);
    ...
    desenha_boss(final_boss, sprites_boss, altura_boss, largura_boss, sprite/5);
    desenha_bala(gun, 0, 0);
    desenha_bala(final_boss->gun, 0, 2);
    if(!personagem->chao)
        desenha_boneco_pulando(sprite_sheet, *personagem, sprite/5);
    else{
        if(personagem->direcao){
